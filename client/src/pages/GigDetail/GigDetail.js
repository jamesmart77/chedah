...
class GigDetail extends React.Component {
    ...
    render() {
        ...
        actions={
         ...
         <Row>
             {this.state.categories.map(cat => <Input name={'goalCategories-' + cat.name } type='checkbox' value={cat.id} ref="check_me" label={cat.name} className='filled-in' onChange={this.handleCategory}/> )}
        </Row>
    </Modal>
